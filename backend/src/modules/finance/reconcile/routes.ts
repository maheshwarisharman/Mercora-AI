 ...
 */
reconcileRouter.post("/missions/:id/reconcile", async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
reconcileRouter.get("/missions/:id/matches", async (req: Request, res: Response): Promise<void> => {
  try {
    const missionId = String(req.params.id);
    const merchantId = (req as any).merchant?.id;
